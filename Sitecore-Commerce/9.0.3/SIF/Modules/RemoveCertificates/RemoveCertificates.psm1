...
function Invoke-RemoveCertificatesTask {
	[CmdletBinding()]
	param(
	...
	try
	{
		$query = "CN=" + $SiteNamePrefix + "*"
		$certificates = Get-ChildItem -Path $Path | Where-Object { $_.Subject -like $query }
		
		Write-Host "Deleted certificate Query $query" -ForegroundColor Green
		
		foreach ($cert in $certificates) {
			$certpath = $Path + "\" + $cert.Thumbprint
			#Get-ChildItem $certpath | Remove-Item
			Write-Host "Deleted certificate Subject=$($cert.Subject) using command below" -ForegroundColor Green
			Write-Host "Get-ChildItem $($certpath) | Remove-Item" -ForegroundColor yellow
			Write-Host " "
		}
	}
	catch
	{
		Write-Host "Unable to delete certificates" -ForegroundColor Red
	}
}

Register-SitecoreInstallExtension -Command Invoke-RemoveCertificatesTask -As RemoveCertificates -Type Task -Force
